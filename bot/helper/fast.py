...
from pyrogram.raw.types import InputMessageID, InputDocumentFileLocation, document_attribute_filename
import pyrogram
...
class FastDownload:
    ...
    def __init__(self, peer_id: int, message_id: int, parts: int, app: pyrogram.Client, file_name: str):
        ...
        message_data = app.send(
            GetMessages(channel=(app.resolve_peer(peer_id=peer_id)), id=[InputMessageID(id=message_id)]))
    ...
    def download_part(self, part):
        ...
        while True:
            try:
                self.func_data[part]["data"] = self.apple.send(
                    GetFile(location=self.document_data, offset=self.part_data[part]["offset"],
                            limit=self.limit_prefix))
                self.done += 1
